...
class FFNet(object):
    ...
    def get_user_url(self, md):
        return self.user_url.format(number=md['authorid'], hostname=self.hostname)

    def get_story_url(self, md):
        return self.story_url.format(number=md['id'], chapter=1, hostname=self.hostname)
    ...
    def download_metadata(self, number):
        ...
        md = self.get_metadata(soup)
        toc = self.get_contents(soup)
        return md, toc

    def compile_story(self, md, toc, outfile, headers=True, contents=False, kindle=False, callback=None, **kwargs):
...
""".format(title=md['title'], id=md['id'], author=md['author']))
        ...
        if headers:
            outfile.write("<h1>{}</h1>\n".format(md['title']))
        if contents:
            outfile.write(self.make_toc(toc))
        for n, t in enumerate(toc):
            x = n + 1
            url = self.story_url.format(hostname=self.hostname, number=md['id'], chapter=x)
            if callback: # For printing progress as it runs.
                callback(n,t)
            r = urlopen_retry(url)
            data = r.read().decode()
            text = self.get_storytext(data)
            if headers:
